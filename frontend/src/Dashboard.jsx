...
export default function Dashboard({ onBack }) {
  ...
  return (
    <main className="min-h-screen p-6 md:p-8" style={{ background: "#0a0a0a", color: "#f0efe8" }}>
    <header className="max-w-7xl mx-auto flex flex-wrap gap-4 items-center justify-between pb-6" style={{ borderBottom: "1px solid #222220" }}>
      <div className="flex items-center gap-3">
        <span className="w-8 h-8 grid place-items-center font-display font-bold" style={{ background: "#d4ff00", color: "#0a0a0a" }}>
          TF
        </span>
        <div>
        ...
        <div className="flex gap-2">
          <button onClick={logout} className="px-3 py-2 text-sm" style={inputStyle}>
            Sign out
          </button>
          <button onClick={onBack} className="px-3 py-2 text-sm" style={inputStyle}>
            Exit
          </button>
      </div>
      </header>
    <div className="max-w-7xl mx-auto grid lg:grid-cols-[260px_1fr] gap-6 pt-6">
      <aside className="p-4" style={{ background: "#111", border: "1px solid #222220" }}>
        <p className="font-display uppercase tracking-widest text-xs mb-3" style={{ color: "#6b6b65" }}>
          Projects
        </p>
        <div className="space-y-1 mb-5">
          {projects.map((project) => 
            <button key={project.id} onClick={() => setActiveProjectId(project.id)} className="w-full text-left p-2 text-sm" style={{ background: project.id === activeProjectId ? "#1a1a1a" : "transparent", color: project.id === activeProjectId ? "#d4ff00" : "#a0a09a" }}>
              {project.name}
            </button>)}
          </div>
          <form onSubmit={createProject} className="space-y-2">
            <input required minLength="3" value={newProject} onChange={(e) => setNewProject(e.target.value)} placeholder="New project" className="w-full p-2 text-sm" style={inputStyle} />
            <button className="w-full p-2 text-xs font-display uppercase" style={{ background: "#d4ff00", color: "#0a0a0a" }}>
              Create project
            </button>
          </form>
      </aside>
      <section>
        {error &&
          <p className="mb-4 p-3 text-sm" style={{ color: "#ff6b6b", border: "1px solid #3a1a1a" }}>
            {error}
          </p>}
        {!activeProject ? 
          <div className="p-10 text-center" style={{ border: "1px solid #222220", color: "#a0a09a" }}>
            {loading ? "Loading projects…" : "Create a project to start tracking work."}
          </div> : <>
          <div className="flex flex-wrap justify-between gap-4 mb-5">
            <div>
              <p className="text-xs uppercase tracking-widest" style={{ color: "#6b6b65" }}>
                Project workspace
              </p>
              ...
              <form onSubmit={createTask} className="flex gap-2 self-end">
                <input required minLength="3" value={newTask} onChange={(e) => 
                  setNewTask(e.target.value)
                  } placeholder="Task title" className="p-2 text-sm" style={inputStyle} />
                <button className="px-3 py-2 text-sm font-display uppercase" style={{ background: "#d4ff00", color: "#0a0a0a" }}>
                  + Task
                </button>
              </form>
            </div>
          <div className="grid md:grid-cols-3 gap-4">
            {COLUMNS.map((column) => 
            <div key={column.key} className="p-3 min-h-80" style={{ background: "#111", border: "1px solid #222220" }}>
              <div className="flex justify-between items-center mb-3">
                <h3 className="font-display uppercase tracking-wider" style={{ color: column.color }}>
                  {column.label}
                </h3>
            ...
            <div className="space-y-2">
              ...
              <article key={task.id} className="p-3" style={{ background: "#0a0a0a", border: "1px solid #222220" }}>
                <p className="text-sm mb-3">
                  {task.title}
                </p>
                <div className="flex gap-2 items-center">
                  <select value={task.status} onChange={(e) => updateTask(task.id, e.target.value)} className="flex-1 p-1 text-xs" style={{ background: "#111", border: "1px solid #222220", color: "#a0a09a" }}>
                    {COLUMNS.map((option) => 
                  ...
                  <button onClick={() => deleteTask(task.id)} className="text-xs" style={{ color: "#ff6b6b" }}>
                    Delete
                  </button>
                </div>
              </article>)}
